import React from 'react'
import Title from '../../components/typography/Title'
import Subtitle from '../../components/typography/Subtitle'
import PrimaryButton from '../../components/ui/PrimaryButton'
import Input from '../../components/ui/Input'
import homeImage from '../../assets/images/home-Image.jpeg'

const HomePreferencePage = () => {
  return (
    <div className='flex gap-x-24 w-full h-full max-[500px]:gap-x-0 max-[500px]:flex-col'>
      <div className='w-1/2 h-screen max-[500px]:h-36 max-[500px]:w-full'>
        <img
          src={homeImage}
          alt='login-back'
          className='w-full h-full object-cover'
        />
      </div>
      <div className='bg-white mr-24 flex  w-1/3 flex-col justify-center max-[500px]:mr-0 max-[500px]:p-2 max-[500px]:w-full'>
        <div className='mb-4'>
          <Title className={'mb-4'}>Select your home Preference</Title>
          <Subtitle className={'mt-4 max-[500px]:w-full max-[500px]:mt-0 '}>
            Help us enhance your home search experience
          </Subtitle>
        </div>
        <form
          autoComplete='new-password'
          className='w-full flex flex-col justify-between h-2/3'
        >
          <div className='w-full my-3 mb-0 max-[500px]:my-0'>
            <label>Home Type</label>
            <div className='flex items-center justify-between mt-2'>
              {[1, 2, 3].map((_, index) => {
                return (
                  <button className='w-36 h-16 border-4 border-primary200 bg-primary200 text-lg text-medium rounded-xl max-[500px]:w-24 max-[500px]:h-12'>
                    {index}
                  </button>
                )
              })}
            </div>
          </div>
          <div className='w-full my-3 '>
            <label>Purchase Type</label>
            <div className='flex items-center justify-between mt-2'>
              {[1, 2, 3].map((_, index) => {
                return (
                  <button className='w-36 h-16 border-4 border-primary200 bg-primary200 text-lg text-medium rounded-xl max-[500px]:w-24 max-[500px]:h-12'>
                    {index}
                  </button>
                )
              })}
            </div>
          </div>
          <div>
            <Input
              label={'Home Location'}
              className={'mt-2'}
              textConfig={{
                placeholder: 'Please',
                type: 'email',
              }}
              divClass={'mt-2'}
            />
            <div className='flex justify-between'>
              <div className='flex flex-col w-[47%]'>
                <label className='text-lg font-medium max-[500px]:text-xs'>
                  Bedroom
                </label>
                <input className='w-full h-14 px-3 outline-none bg-white border-2 border-black rounded-xl mt-4 max-[500px]:mt-2 max-[500px]:h-11' />
              </div>
              <div className='flex flex-col w-[47%]'>
                <label className='text-lg font-medium max-[500px]:text-xs'>
                  Bathroom
                </label>
                <input className='w-full h-14 px-3 outline-none bg-white border-2 border-black rounded-xl mt-4 max-[500px]:mt-2 max-[500px]:h-11' />
              </div>
            </div>
          </div>
          <PrimaryButton className={'mt-10'}>Procees</PrimaryButton>
        </form>
      </div>
    </div>
  )
}

export default HomePreferencePage
